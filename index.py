...
def result():
    ...
    result = main(userText)
    print("data : "+result)
    return render_template('home.html', transcript=result)
...
def detectIndexOfeachVerbs(doc):
    indexDepart = 0
    indexArriver = 0
    for item in doc:
        if(item.pos_ == "VERB"):
...
def main(userText):
    ...
    doc = nlp(text) #lower
    display(doc)
    wordsTokenisation(doc)
    saveVerbsInArray(doc)
    saveAllCitiesInArray(doc)
    indexVerb1,indexVerb2 = detectIndexOfeachVerbs(doc)[0],detectIndexOfeachVerbs(doc)[1]
    depart, arrival = foundDepartureCountry(indexVerb1,indexVerb2)
    resultat = "Départ : "+depart+"-> Arriver : "+arrival
